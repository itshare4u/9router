...
describe("OpenAI Responses streaming termination", () => {
  ...
  it("marks non-streaming conversion as failed when EOF arrives before response.completed", async () => {
    const json = await convertResponsesStreamToJson(responseStream([
      "event: response.created",
      `data: ${JSON.stringify({ type: "response.created", response: { id: "resp_test", status: "in_progress" } })}`,
      "",
    ].join("\n")));
